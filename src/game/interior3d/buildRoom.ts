...
export interface AABB {
  ...
  /** Optional live gate for moving geometry such as doors. */
  isActive?: () => boolean;
}
...
export function buildRoom(kind: RoomKind): RoomBuildResult {
  ...
  if (kind === "medical") {
    // ── 医学楼视觉细节（全部不参与碰撞，保持原有玩法与通行空间）──
    const groutMat = trackMat(new THREE.MeshBasicMaterial({ color: 0x708087, transparent: true, opacity: 0.2 }));
    const routeMat = trackMat(new THREE.MeshBasicMaterial({ color: 0x6f2929, transparent: true, opacity: 0.42 }));
    const signMat = stdMat(0x315b60, 0.62, 0.08);
    const darkSignMat = stdMat(0x182327, 0.76, 0.04);
    const chromeMat = stdMat(0x7c898d, 0.25, 0.72);
    ...
    const stainMat = trackMat(new THREE.MeshBasicMaterial({
      color: 0x42191b,
      transparent: true,
      opacity: 0.32,
      depthWrite: false,
    ...
    // 老式水磨石地砖、正门导向线与墙脚防撞带。
    for (let x = -3.6; x <= 3.6; x += 1.2) addBox(0.012, 0.008, ROOM_L - 0.65, x, 0.01, 0, groutMat, false);
    for (let z = -6.9; z <= 6.9; z += 1.15) addBox(ROOM_W - 0.65, 0.008, 0.012, 0, 0.011, z, groutMat, false);
    addBox(0.055, 0.01, 7.4, 0, 0.018, 3.05, routeMat, false);
    addBox(0.08, 0.56, ROOM_L - 0.7, -halfW + 0.19, 0.52, 0, signMat, false);
    addBox(0.08, 0.56, ROOM_L - 0.7, halfW - 0.19, 0.52, 0, signMat, false);

    // 正门导诊牌：用几何色块模拟楼层目录，避免引入字体资源。
    addBox(2.7, 0.72, 0.05, 0, 2.34, halfL - 0.2, darkSignMat, false);
    addBox(2.45, 0.09, 0.018, 0, 2.52, halfL - 0.16, signMat, false);
    for (let i = 0; i < 3; i++) {
      addBox(0.28, 0.07, 0.018, -0.98, 2.33 - i * 0.13, halfL - 0.16, whiteMat, false);
      addBox(1.55, 0.035, 0.018, 0.22, 2.33 - i * 0.13, halfL - 0.16, whiteMat, false);
    }

    // 乳白色吊顶灯箱，让病床、隔断和红色指引在中景都能辨认。
    for (const z of [-5.2, -0.6, 4.0]) {
      addBox(1.55, 0.06, 0.42, -1.6, WALL_H - 0.08, z, whiteMat, false);
      addBox(1.55, 0.06, 0.42, 1.6, WALL_H - 0.08, z, whiteMat, false);
      const light = new THREE.PointLight(0xbad3d0, 0.34, 4.8, 2.1);
      light.position.set(0, WALL_H - 0.25, z);
      root.add(light);
    }

    const addIvStand = (x: number, z: number): void => {
      const pole = new THREE.Mesh(track(new THREE.CylinderGeometry(0.018, 0.022, 1.65, 10)), chromeMat);
      pole.position.set(x, 0.84, z);
      root.add(pole);
      const hook = new THREE.Mesh(track(new THREE.TorusGeometry(0.12, 0.015, 6, 14, Math.PI)), chromeMat);
      hook.position.set(x, 1.66, z);
      hook.rotation.z = Math.PI / 2;
      root.add(hook);
      const bag = addBox(0.2, 0.34, 0.055, x + 0.13, 1.43, z, glassMat, false);
      bag.rotation.z = -0.06;
      for (const angle of [0, (Math.PI * 2) / 3, (Math.PI * 4) / 3]) {
        const foot = new THREE.Mesh(track(new THREE.CylinderGeometry(0.014, 0.014, 0.42, 8)), chromeMat);
        foot.position.set(x + Math.cos(angle) * 0.15, 0.05, z + Math.sin(angle) * 0.15);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // 枕头、床栏、床尾记录夹与输液架仅作视觉补充。
      addBox(0.72, 0.1, 0.38, -halfW + 1.3, 0.73, z - 0.62, whiteMat, false);
      addBox(0.035, 0.32, 1.25, -halfW + 0.82, 0.86, z + 0.05, chromeMat, false);
      addBox(0.035, 0.32, 1.25, -halfW + 1.78, 0.86, z + 0.05, chromeMat, false);
      addBox(0.42, 0.34, 0.035, -halfW + 1.3, 1.03, z + 0.96, darkSignMat, false);
      addBox(0.28, 0.035, 0.018, -halfW + 1.3, 1.06, z + 0.94, whiteMat, false);
      addIvStand(-halfW + 2.1, z + 0.5);
    }
    ...
    // 导诊台上的病历夹、药瓶和磨砂隔板。
    addBox(0.42, 0.035, 0.28, halfW - 2.8, 0.84, 4.15, darkSignMat, false).rotation.y = -0.18;
    for (let i = 0; i < 3; i++) {
      const bottle = new THREE.Mesh(track(new THREE.CylinderGeometry(0.035, 0.045, 0.14 + i * 0.025, 10)), i === 1 ? glassMat : whiteMat);
      bottle.position.set(halfW - 2.35 + i * 0.11, 0.88, 4.12);
      root.add(bottle);
    }
    addBox(1.0, 0.72, 0.025, halfW - 2.6, 1.18, 4.52, glassMat, false);

    // 地面拖痕与一串稀薄污迹，加强旧医学楼的使用痕迹，但不改变任何触发区。
    for (const [x, z, r, sx] of [
      ...
      const stain = new THREE.Mesh(track(new THREE.CircleGeometry(r, 20)), stainMat);
      stain.rotation.x = -Math.PI / 2;
      stain.scale.set(sx, 1, 1);
      stain.position.set(x, 0.024, z);
      root.add(stain);
    }
    ...
    const medDivider = new RoomDivider({
      ...
      thickness: 0.2,
      color: palette.wall,
      // 1.45 m leaves a reliable player-width passage after collider padding;
      // the old 1.0 m gap was narrower than the camera's normal walk sway.
      door: { position: 0.5, width: 1.45, height: 2.2, keyItemId: "key_card", label: "地下仓库（需要门禁卡）" },
    });
    ...
    // 隔间内只保留靠墙储物柜；中央不再放置解剖台，
    // 让入口、门洞与红色剧情点之间保持完全直通。
    addBox(1.2, 0.5, 2.0, 2.3, 0.35, -halfL + 1.6, metalMat);
    addBox(1.2, 0.5, 2.0, -2.3, 0.35, -halfL + 1.6, metalMat);
    // 仓库门头、警示条和封存箱细节。
    addBox(2.1, 0.34, 0.045, 0, 2.62, -halfL + 5.08, darkSignMat, false);
    for (let i = 0; i < 5; i++) addBox(0.23, 0.045, 0.018, -0.75 + i * 0.38, 2.62, -halfL + 5.04, routeMat, false);
    for (const x of [-2.65, 2.65]) {
      addBox(0.9, 0.52, 0.72, x, 0.27, -halfL + 1.8, woodMat, false);
      addBox(0.72, 0.025, 0.5, x, 0.55, -halfL + 1.8, darkSignMat, false);
    }
