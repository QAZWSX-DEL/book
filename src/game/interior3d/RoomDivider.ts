...
export class RoomDivider {
  ...
  constructor(config: RoomDividerConfig) {
    ...
    const pnz = ndx;
    // BoxGeometry's long axis is local +X. Rotate that axis onto the wall's
    // XZ direction; the previous swapped atan2 made every horizontal divider
    // render vertically while its collision boxes stayed horizontal.
